...
def main():
    ...
    ap.add_argument('results_file', help='File to save model results')
    ...
    precomputed_frame_to_orientation_to_car_counts = {}
    precomputed_frame_to_orientation_to_person_counts = {}
    if os.path.exists(args.results_file):
        ...
        for idx, row in df.iterrows():
            ...
            car_count = int(row['cars'])
            people_count = int(row['people'])
            if frame not in precomputed_frame_to_orientation_to_car_counts:
                precomputed_frame_to_orientation_to_car_counts[frame] = {}
            if frame not in precomputed_frame_to_orientation_to_person_counts:
                precomputed_frame_to_orientation_to_person_counts[frame] = {}
            precomputed_frame_to_orientation_to_car_counts[frame][orientation] = car_count
            precomputed_frame_to_orientation_to_person_counts[frame][orientation] = people_count
            
    perform_continual_learning = True
    with open(args.results_file, file_mode) as f_results:
        if file_mode == 'w':
            f_results.write('frame,orientation,cars,people\n')
        for current_frame in range(args.frame_begin, args.frame_limit+1):
            ...
            if result_idx != prev_result_idx:
                ...
                orientation_to_accuracy = {}


                perform_continual_learning = True
                # Get best fixed orientation
                for f in range(sub_frame_begin, sub_frame_limit):
                    ...
                    for o in frame_to_actual_orientation_to_count[f]:
                        if frame_to_actual_orientation_to_count[f][o] > highest_count:
                            highest_count = frame_to_actual_orientation_to_count[f][o]
                    for o in frame_to_actual_orientation_to_count[f]:
                        if o not in orientation_to_accuracy:
                            orientation_to_accuracy[o] = 0.0
                        orientation_to_accuracy[o] += frame_to_actual_orientation_to_count[f][o] / highest_count

                print('selecting best fixed from ')
                print(orientation_to_count)
                max_count = -1
                neighboring_orientations = []
                for o in orientation_to_accuracy:
                    if orientation_to_accuracy[o] > max_count:
                        max_count = orientation_to_accuracy[o]
                        best_fixed_orientation = o
                    neighboring_orientations.append(o)
...
#            neighboring_orientations = generate_neighboring_orientations(best_fixed_orientation)
            ...
            if current_frame in precomputed_frame_to_orientation_to_car_counts and current_frame in precomputed_frame_to_orientation_to_person_counts:
                # Check to see if results were already computed
                for o in orientation_to_file:
                    if o in precomputed_frame_to_orientation_to_car_counts:
                        orientation_to_car_count = precomputed_frame_to_orientation_to_car_counts[o]
                    else:
                        orientation_to_car_count, orientation_to_person_count = run_inference(orientation_to_file,  model, args.device)
                        break

                    if o in precomputed_frame_to_orientation_to_person_counts:
                        orientation_to_person_count = precomputed_frame_to_orientation_to_person_counts[o]
                    else:
                        orientation_to_car_count, orientation_to_person_count = run_inference(orientation_to_file,  model, args.device)
                        break
                # Some hacky thing that does not perform continual learning if precompted results are detceted
                perform_continual_learning = False
            else:
                orientation_to_car_count, orientation_to_person_count = run_inference(orientation_to_file,  model, args.device)
            ...
            for o in orientation_to_car_count:
                result_str += f'{current_frame},{o},{orientation_to_car_count[o]},{orientation_to_person_count[o]}\n'
#                result_str += f'{o}:{orientation_to_car_count[o]},'

            f_results.write(result_str )
...
#                predicted_orientation_to_ranking[o] = round( ( orientation_to_model_ranking[o] + orientation_to_latest_ranking[o] ) / 2)
            ...
            if total_frames % 50 == 0 and len(orientation_to_training_frames) > 0 and perform_continual_learning:
                ...
                num_retraining_images = 60
                largest_len = 0
                for o in orientation_to_training_frames:
                    if len(orientation_to_training_frames[o]) > largest_len:
                        largest_len = len(orientation_to_training_frames[o])

                for o in orientation_to_training_frames:
                    while len(orientation_to_training_frames[o]) < largest_len:
