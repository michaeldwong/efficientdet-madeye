...
#        orientations = frame_limit_to_orientation[sub_frame_limit][:1]
        ...
        val_upper_bound = int((frame_bounds[result_idx][1] - frame_bounds[result_idx][0]) * 0.35)
...
#        for o in all_orientations:
        for o in generate_random_orientations():
            ...
            rectlinear_file = os.path.join(args.rectlinear, o, f'frame{current_frame}.jpg')
            if os.path.getsize(inference_file) > 0:
                ...
                car_objects, person_objects = extract_csv_results(inference_file, CAR_CONFIDENCE_THRESH, PERSON_CONFIDENCE_THRESH)
                f.write(f'{current_frame},{o},{rectlinear_file},{len(car_objects)},{len(person_objects)}\n')
        current_frame += 1
